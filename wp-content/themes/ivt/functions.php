...
/**
 * display thumbnails in the post
 */
add_theme_support( 'post-thumbnails' ); // for all types of posts
/* Adding a fixed size to thumbnails */
...
/** The latest notes
------------------------------------------------------
$post_num (5) = amount of links
$format ('') = {avatar} {author}: {date:j.M.Y} - {a}{title}{/a} ({comments})
$cat ('') = Categories from which you want to display (5,15) or to exclude (-5,-15), 
63/5000
separated by commas (both inclusion and exclusion does not work
$list_tag (li) = tag of the list
*/
function get_ivt_recent_posts ($post_num=3, $format='', $cat='', $list_tag='li', $echo=true){
	...
	// eliminate the main query (wp_query)
	...
	if( strpos($format, '{author}')!==false ){
		...
		//if neede to show avatar(gavatar)
